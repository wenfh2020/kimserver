...
bool SessionMgr::add_session(Session* s) {
    ...
    if (it != m_sessions.end()) {
        s->set_active_time(get_net()->get_time_now());
        return true;
    }
    m_sessions[sessid] = s;
    ...
    if (w == nullptr) {
        ...
        LOG_ERROR("module add session(%s) timer failed!", s->get_name());
        return false;
    }
    s->set_timer(w);
    return true;
...
void SessionMgr::on_session_timer(void* privdata) {
    ...
    double secs = s->get_keep_alive() - (get_net()->get_time_now() - s->get_active_time());
    if (secs > 0) {
