...
void MysqlAsyncConn::ping_start() {
    ...
    if (ret == 0) {
        ...
        LOG_DEBUG("ping falied! error: %d, errstr: %s",
...
void MysqlAsyncConn::handle_task() {
    ...
    if (error == CR_SERVER_LOST || error == CR_SERVER_GONE_ERROR) {
        ...
        if (error != 0) {
            if (m_cur_task != nullptr) {
                LOG_ERROR("handle task error: %d, %s, sql: %s",
                          error, errstr, m_cur_task->sql.c_str());
            } else {
                LOG_ERROR("handle task error: %d, %s", error, errstr);
            }
        }
        if (m_is_connected) {
            set_state(STATE::WAIT_OPERATE);
